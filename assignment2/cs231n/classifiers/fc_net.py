...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dz, grads['W2'], grads['b2'] = affine_backward(dL, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, h in enumerate(hidden_dims):
            l = i + 1
            self.params[('b%d') % l] = np.zeros(h)
            self.params[('W%d') % l] = np.random.normal(loc=0.0, scale=weight_scale, size=(prev_dim, h))
            
            if use_batchnorm:
                self.params[('gamma%d') % l] = np.ones(h)
                self.params[('beta%d') % l] = np.zeros(h)
            
            prev_dim = h

        self.params[('b%d') % self.num_layers] = np.zeros(num_classes)
        self.params[('W%d') % self.num_layers] = np.random.normal(loc=0.0, scale=weight_scale, size=(prev_dim, num_classes))
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        def batch_norm_relu_forward(x, w, b, gamma, beta, bn_param):
        ...
        for i in range(1, self.num_layers):
            if self.use_batchnorm:
                ...
                input, caches[i] = batch_norm_relu_forward(input, w, b, gamma, beta, self.bn_params[i-1])
            else:
                W = self.params['W%d' % i]
                b = self.params['b%d' % i]
                input, caches[i] = affine_relu_forward(input, W, b)
            if self.use_dropout:
        ...
        scores, caches[self.num_layers] = affine_forward(input, W, b)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        for i in range(1, self.num_layers+1)[::-1]:
            ...
            if self.use_batchnorm:
                if i == self.num_layers:
                    dx, grads['W%d' % i], grads['b%d' % i] = affine_backward(dx, caches[i])
                else:
                    if self.use_dropout:
                        dx = dropout_backward(dx, dropout_caches[i])
                    dx, grads['W%d' % i], grads['b%d' % i], grads['gamma%d' % i], grads['beta%d' % i] = batch_norm_relu_backward(dx, caches[i])
            else:
                if i == self.num_layers:
                    dx, grads['W%d' % i], grads['b%d' % i] = affine_backward(dx, caches[i])
                else:
                    if self.use_dropout:
                        dx = dropout_backward(dx, dropout_caches[i])
                    dx, grads['W%d' % i], grads['b%d' % i] = affine_relu_backward(dx, caches[i])
